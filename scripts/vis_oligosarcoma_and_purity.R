...
# fig 4g: spike-in paths re-styled by pim ----
...
plt.tumor <- rbind(
  glass_od.metadata.array_samples |> 
    dplyr::filter(resection_id %in% c("0017-R3","0008-R2","0121-R3","0054-R3")) |> 
    dplyr::rename(array_tumor_id = resection_id) |> 
    dplyr::select(array_tumor_id, array_sentrix_id, 
                  ...
                  array_mnp_predictBrain_v12.8_cal_class, array_methylation_bins_1p19q_purity) |> 
    dplyr::mutate(array_fraction_normal_brain = 0) |> 
    dplyr::mutate(array_control_id = "s107"),
  glass_od.metadata.array_samples |> 
    dplyr::filter(resection_id %in% c("0017-R3","0008-R2","0121-R3","0054-R3")) |> 
...
plt.spiked <- cortex_dilution.metadata.array_samples |> 
  dplyr::select(array_tumor_id, array_sentrix_id,
                ...
                array_mnp_predictBrain_v12.8_cal_class, array_methylation_bins_1p19q_purity,
                array_fraction_normal_brain_est, array_control_id) |> 
  dplyr::rename(array_fraction_normal_brain = array_fraction_normal_brain_est)

plt.normal_cortex <- rbind(
  normal_cortex.metadata.array_samples |> 
    dplyr::filter(!is.na(array_control_id)) |> 
    dplyr::select(array_control_id, array_sentrix_id,
                  array_mnp_predictBrain_v12.8_cal_A_IDH_HG,
                  array_mnp_predictBrain_v12.8_cal_OLIGOSARC_IDH,
                  array_mnp_predictBrain_v12.8_cal_CTRL_HEMI,
...
plt <- rbind(plt.tumor, plt.spiked, plt.normal_cortex) |> 
  dplyr::mutate(array_mnp_predictBrain_v12.8_cal_class = ifelse(array_mnp_predictBrain_v12.8_cal_class %in% c("O_IDH", "A_IDH_HG", "A_IDH_LG","OLIGOSARC_IDH") == F, "Other", array_mnp_predictBrain_v12.8_cal_class)) |> 
  tidyr::pivot_longer(cols=c(array_mnp_predictBrain_v12.8_cal_A_IDH_HG,
                             array_mnp_predictBrain_v12.8_cal_OLIGOSARC_IDH,
                             array_mnp_predictBrain_v12.8_cal_CTRL_HEMI,
  ), names_to = "prediction_class", values_to = "prediction_probability") |> 
  dplyr::mutate(prediction_class = gsub("array_mnp_predictBrain_v12.8_cal_","",prediction_class)) |> 
  dplyr::mutate(grp = paste0(prediction_class,"_",array_fraction_normal_brain))
...
ggplot(plt, aes(x=array_fraction_normal_brain, y=prediction_probability, col=prediction_class)) +
  ...
  labs(x="Dilution fraction",
       y = "MNP 12.8 calibrated probability", 
       col="",
