...
data2 <- data
data2$nas <- rowSums(is.na(data2))
data2 <- data2 |> 
  dplyr::filter(nas < 20) |> 
  dplyr::mutate(nas=NULL)


data2 <- t(scale(t(data2), center = TRUE, scale = TRUE)) #
...
plt <- stats.time |> 
  dplyr::left_join(stats.grade, by=c('peptide_id'='peptide_id'), suffix=c('.time', '.grade')) |> 
  dplyr::left_join(stats.cgc, by=c('peptide_id'='peptide_id'), suffix=c('', '.cgc')) |> 
  dplyr::left_join(metadata.proteomics.proteins, by=c('peptide_id'='peptide_id')) |> 
...
plt <- plt |> dplyr::mutate(label = Genes %in% c("IDH1"))
p1 = ggplot(plt, aes(x=t, y=t.grade, label=Genes)) +
...
plt <- plt |> dplyr::mutate(label = Genes %in% c("IDH2"))
p2 = ggplot(plt, aes(x=t, y=t.grade, label=Genes)) +
  geom_hline(yintercept=0, col="darkgray", lwd=theme_nature_lwd) +
  geom_vline(xintercept=0, col="darkgray", lwd=theme_nature_lwd) +
  geom_point(data=subset(plt, label == F), pch=19, cex=0.1, col="black", alpha=0.2) +
  geom_point(data=subset(plt, label == T), pch=19, cex=0.6, col="red") +
...
plt <- plt |> dplyr::mutate(label = Genes %in% c("GLUD1"))
p3 = ggplot(plt, aes(x=t, y=t.grade, label=Genes)) +
  geom_hline(yintercept=0, col="darkgray", lwd=theme_nature_lwd) +
  geom_vline(xintercept=0, col="darkgray", lwd=theme_nature_lwd) +
  geom_point(data=subset(plt, label == F), pch=19, cex=0.1, col="black", alpha=0.2) +
  geom_point(data=subset(plt, label == T), pch=19, cex=0.6, col="red") +
...
plt <- plt |> dplyr::mutate(label = Genes %in% c("FN1"))
p4 = ggplot(plt, aes(x=t, y=t.grade, label=Genes)) +
  geom_hline(yintercept=0, col="darkgray", lwd=theme_nature_lwd) +
  geom_vline(xintercept=0, col="darkgray", lwd=theme_nature_lwd) +
  geom_point(data=subset(plt, label == F), pch=19, cex=0.1, col="black", alpha=0.2) +
  geom_point(data=subset(plt, label == T), pch=19, cex=0.6, col="red") +
...
plt <- plt |> dplyr::mutate(label = Genes %in% plt$Genes[2200])
ggplot(plt, aes(x=t, y=t.grade, label=Genes)) +
  geom_hline(yintercept=0, col="darkgray", lwd=theme_nature_lwd) +
  geom_vline(xintercept=0, col="darkgray", lwd=theme_nature_lwd) +
  geom_point(data=subset(plt, label == F), pch=19, cex=0.1, col="black", alpha=0.2) +
  geom_point(data=subset(plt, label == T), pch=19, cex=0.6, col="red") +
...
plt |> 
  dplyr::filter(adj.P.Val.grade < 0.01) |> 
  dplyr::pull(Genes) |> 
  unique() |> 
  sort()
