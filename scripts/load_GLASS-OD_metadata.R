...
glass_od.metadata.patients <- DBI::dbReadTable(metadata.db.con, 'view_patients') |> 
  (function(.) {
    print(dim(.))
    assertthat::assert_that(nrow(.) == 127 + 57 + 7 + 4)
    return(.)
  })()  |> 
...
glass_od.metadata.patients <- glass_od.metadata.patients |> 
  ...
  (function(.) {
    print(dim(.))
    assertthat::assert_that(nrow(.) ==  126 + 57 + 7 + 4) # + 10x astro
    return(.)
  })() |> 
...
tmp <- DBI::dbReadTable(metadata.db.con, 'view_array_samples') |> 
  ...
  (function(.) {
    print(dim(.))
    assertthat::assert_that(sum(is.na(.$arraychip_date)) <= (6 + 57 + 17 + 4))
    return(.)
  })() |> 
...
tmp <- query_mnp_12.8_CNVP_segment_csv(
  ...
  CONST_N_GLASS_OD_ALL_SAMPLES + CONST_N_CATNON_ALL_SAMPLES + 1, 
...
tmp <- readRDS("cache/analysis_EPITOC2.Rds") |> 
  ...
  (function(.) {
    print(dim(.))
    assertthat::assert_that(nrow(.) == 210)# CONST_N_GLASS_OD_INCLUDED_SAMPLES -  only HQ samples - should become 210?
    return(.)
  })()
